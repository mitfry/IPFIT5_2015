...
def processDirectories(theDir, useHash):
    ...
    ProcessCount = 0
    ErrorCount = 0
    oCVS = _CSVWriter(folder_csv + 'fileSystemReport.csv', useHash)
    ...
    if os.path.exists(theFile):
        ...
        if not os.path.islink(theFile):
            ...
            if os.path.isfile(theFile):
                ...
                print "================================\n"
                test.writeCSVRow(simpleName, size, hashType, hexOfHash, mtime, atime, ctime)
                return True
...
if __name__ == '__main__':
    ...
    folder_csv = raw_input('Waar mag het csv komen?')
...
class _CSVWriter:
    def __init__(self, fileName, hashType):
        try:
            ...
            self.writer.writerow(('File', 'Size', hashType, 'Hash', 'Last Modified', 'Last Access', 'Created Time'))
        except:
            return

    def writeCSVRow(self, filePath, fileSize, hashType, hashVal, aTime, cTime):
        self.writer.writerow((filePath, fileSize, hashType, hashVal, aTime, cTime, hashVal))
